...
namespace Demo.Common.ExtensionMethods
{
    ...
    public static class XmlExtensions
    {
        ...
        public static bool IsValidXml(this string source)
        {
            ...
            using (var xmlReader = XmlReader.Create(new StringReader(temp), ReaderSettings))
            {
                try
                {
            ...
            bool omitXmlDeclaration = false, bool darkStyle = false)
        {
            ...
            var tempRtf = indentXml ? source.ToIndentXml(omitXmlDeclaration) : source;

            var highlightStringBuilder = new StringBuilder(string.Empty);
            ...
            for (var i = 0; i < tempRtf.Length; i++)
            {
            ...
            var result = highlightStringBuilder.ToString();

            var colorTableStartIndex = result.IndexOf("{\\colortbl;");

            if (colorTableStartIndex != -1)
            {
                var colorTableEndIndex = result.IndexOf('}', colorTableStartIndex);
            ...
            {
                var rtfIndex = result.IndexOf("\\rtf");
                ...
                {
                    var insertIndex = result.IndexOf('{', rtfIndex);
        ...
        {
            var document = source is XmlDocument ? (XmlDocument) source : source.OwnerDocument;
        ...
        {
            var document = sourceNode is XmlDocument ? (XmlDocument) sourceNode : sourceNode.OwnerDocument;
            XmlNode node = document.CreateElement(childNode);
        ...
        {
            return GetAttribute(source, attributeName, default(T));
        }
        ...
        public static T GetAttribute<T>(this XmlNode source, string attributeName, T defaultValue)
        {
            ...
            return !value.IsNullOrEmpty() ? value.ConvertTo(defaultValue) : defaultValue;
        }
