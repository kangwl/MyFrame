...
namespace Demo.Common.ExtensionMethods
{
    ...
    public static class IOExtensions
    {
        ...
        public static string RenameFile(this string filename)
        {
            ...
            var pattern = "^" + filenameWithoutExtension + "\\s*(\\d+)?";
            var regex = new Regex(pattern);
            var count =
                Directory.GetFiles(directoryName, filenameWithoutExtension + "*" + fileExtension)
                    .Where(i => regex.IsMatch(Path.GetFileNameWithoutExtension(i)))
                    .Count();

            return filenameWithoutExtension + " (" + (count + 1) + ")" + fileExtension;
        }
        ...
        public static string RenameFolder(string path)
        {
            ...
            var pattern = "^" + folderName + "\\s*(\\d+)?";
            var regex = new Regex(pattern);
            var count =
                Directory.GetDirectories(directoryName, folderName + "*")
                    .Where(i => regex.IsMatch(Path.GetFileName(i)))
                    .Count();
            ...
            Encoding encoding = null)
        {
            ...
            var fullDirectoryPath = Path.GetDirectoryName(fullPath);
        ...
        public static string MoveFileTo(this string sourceFileName, string destFileName, bool overwrite = false)
        {
            ...
            var sourceFullPath = Path.GetFullPath(sourceFileName);

            var destFullPath = Path.GetFullPath(destFileName);

            var destFullDirectoryPath = Path.GetDirectoryName(destFileName);
            ...
            bool throwOnError = false)
        {
            ...
            var destFullPath = Path.GetFullPath(destDirectory);
        ...
        public static bool ExistsFileSystem(this string source)
        {
            ...
            var isDirectory = fileInfo.Attributes != (FileAttributes) (-1) &&
        ...
        /// </param>
        [EnvironmentPermission(SecurityAction.Demand, Unrestricted = true)]
        public static void ExecuteCmdLine(this string sourceCmd, bool runasAdmin = true, bool hidden = true,
            int milliseconds = 0)
        {
