...
namespace Demo.Common.ExtensionMethods
{
    ...
    public static class ByteExtensions
    {
        ...
        {
            var result = new byte[(source.Length - 1)/8 + 1];
        ...
        {
            var result = new BitArray(source);
        ...
        public static BitArray ToBitArray(this byte source)
        {
            var result = new BitArray(new[] {source});
        ...
        public static string ToHexString(this IList<byte> source, char? delimiter = null)
        {
            ...
            var result = new char[source.Count*2];
            ...
            if (delimiter != null)
            {
                var resultWithDelimiter = new char[source.Count*3];
        ...
        {
            var temp = source.RemoveAny(false, ' ', '-', '\n', '\r');
            ...
            var result = new byte[temp.Length/2];
            ...
            for (int bx = 0, sx = 0; bx < result.Length; ++bx, ++sx)
            {
                ...
                result[bx] =
                    (byte)
                        ((tempChar > '9' ? (tempChar > 'Z' ? tempChar - 'a' + 10 : tempChar - 'A' + 10) : tempChar - '0') <<
                         4);
