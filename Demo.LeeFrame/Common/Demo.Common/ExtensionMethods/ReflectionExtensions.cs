...
namespace Demo.Common.ExtensionMethods
{
    ...
    public static class ReflectionExtensions
    {
        ...
        public static bool IsInheritFrom(this Type source, Type baseType)
        {
            ...
            foreach (var item in source.GetInterfaces())
            {
        ...
        {
            if (source != typeof (string) && source.GetInterface("IEnumerable") != null &&
                source.GetInterface("IDictionary") == null)
            {
                if (source.IsArray)
                {
        ...
        public static object CreateInstanceGeneric(this Type source, Type[] typeArguments, params object[] args)
        {
            ...
            var genericType = source.MakeGenericType(typeArguments);
            ...
            params object[] parameters)
        {
            ...
            try
            {
                methodInfo = source.GetType()
                    .GetMethod(method, isPublicOnly ? PublicOnlyLookup : AllLookup, null, CallingConventions.Any,
                        parameterTypes, null);
            }
            catch (AmbiguousMatchException)
            {
                var methods =
                    source.GetType()
                        .GetMethods(isPublicOnly ? PublicOnlyLookup : AllLookup)
                        .Where(p => p.Name.Equals(method, StringComparison.OrdinalIgnoreCase) && !p.IsGenericMethod);
            ...
            params object[] index)
        {
            return
                source.GetType()
                    .GetProperty(propertyName, isPublicOnly ? PublicOnlyLookup : AllLookup)
        ...
        public static Dictionary<string, object> RetrieveProperties(this object source, bool isPublicOnly = true)
        {
            ...
            foreach (var property in source.GetType().GetProperties(isPublicOnly ? PublicOnlyLookup : AllLookup))
            {
                if (property.CanRead)
                {
        ...
        {
            return source.IsGenericType && source.GetGenericTypeDefinition() == typeof (Nullable<>) &&
                   Nullable.GetUnderlyingType(source).CanConvert();
        }
    }
}