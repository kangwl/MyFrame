...
namespace Demo.Common.Reflection
{
    ...
    public class PropertyAccessor
    {
        ...
        /// </summary>
        private readonly Dictionary<string, GetFunction<object, object>> _getFunctionDictionary;

        /// <summary>
        ///     Field _setActionDictionary.
        /// </summary>
        private readonly Dictionary<string, SetAction<object, object>> _setActionDictionary;
        ...
        public PropertyAccessor(Type targetType, bool ignoreCase = true)
        {
            ...
            _getFunctionDictionary =
                new Dictionary<string, GetFunction<object, object>>(ignoreCase
                    ? StringComparer.OrdinalIgnoreCase
                    : StringComparer.Ordinal);

            _setActionDictionary =
                new Dictionary<string, SetAction<object, object>>(ignoreCase
                    ? StringComparer.OrdinalIgnoreCase
                    : StringComparer.Ordinal);
            ...
            foreach (var item in TargetType.GetProperties())
            {
                ...
                {
                    _getFunctionDictionary[item.Name] =
                        CreateGetFunction(TargetType.GetMethod(string.Format("get_{0}", item.Name)));

                    _setActionDictionary[item.Name] =
                        CreateSetAction(TargetType.GetMethod(string.Format("set_{0}", item.Name)));
                }
            }
        }
        ...
        public object GetProperty(object targetObject, string propertyName)
        {
            ...
            CheckType(targetObject, "targetObject");

            return _getFunctionDictionary[propertyName](targetObject);
        }
        ...
        public void SetProperty(object targetObject, string propertyName, object value)
        {
            ...
            _setActionDictionary[propertyName](targetObject, value);
        }
        ...
        private void CheckType(object value, string parameterName)
        {
            ...
            {
                throw new ArgumentException("The target type cannot be assignable from the type of given object.",
                    parameterName);
            }
        }
        ...
        private SetAction<object, object> CreateSetAction(MethodInfo setMethod)
        {
            ...
            var method = new DynamicMethod("SetValue", null, new[] {typeof (object), typeof (object)});
            ...
            if (paramType.IsValueType)
            {
                ...
                {
                    var load = ValueTypeOpCodeDictionary[paramType];
