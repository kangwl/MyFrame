...
namespace Demo.Common.Reflection
{
    ...
    internal static class InternalLogger
    {
        ...
        /// </summary>
        private static readonly string ExecutingAssembly =
            Path.GetFullPath(new Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath);

        /// <summary>
        ///     Field GlobalDebugFlagFile.
        /// </summary>
        private static readonly string GlobalDebugFlagFile = Path.Combine(Path.GetDirectoryName(ExecutingAssembly),
            "DevLib#Debug");
        ...
        {
            var stackFrame = new StackFrame(skipFrames < 1 ? 1 : skipFrames + 1, true);
        ...
        private static void AppendToFile(string message)
        {
            ...
            finally
            {
                ...
                try
                {
                    fileStream = new FileStream(LogFile, FileMode.OpenOrCreate, FileAccess.ReadWrite,
