...
namespace Demo.DataCenter.Dapper.Ext
{
    ...
    public static class ExpressFunc
    {
        ...
        public static void Where<T>(this DataFactory dataFactory, Expression<Func<T, bool>> func)
        {
            if (func.Body is BinaryExpression)
            {
                BinaryExpression be = ((BinaryExpression) func.Body);
                // dataFactory.WhereStr = BinarExpressionProvider(be.Left, be.Right, be.NodeType);
            }
            else
            {
                // dataFactory.WhereStr = string.Empty;
            }
